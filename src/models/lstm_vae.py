...
from .lstm_vae_utils import LSTM_VAE, fit_lstm_vae_early_stopping, predict_lstm_var_test_scores
...
class LSTM_VAE_Algo(Algorithm, PyTorchUtils):
    def __init__(self, name: str='lstm_vae', num_epochs: int=100, batch_size: int=128, 
                ...
                gpu: int=None, patience: int=20, save_dir='data'):
    ...
    def compute_mean_std(self, train_data):
        ...
        with torch.no_grad():
            for ts_batch in whole_data_loader:
                ...
                _, _, log_px, _ = self.model(ts_batch[:, -1])
    ...
    def fit(self, train_data : pd.DataFrame, categorical_columns=None):
        ...
        self.model = LSTM_VAE(n_features=train_data_win.shape[2], hidden_size=self.hidden_size,
                              lstm_dim=self.lstm_dim, seed=self.seed, gpu=self.gpu)
        print(f"Fitting {self.name} model")
        writer = SummaryWriter(self.save_dir)

        self.model = fit_lstm_vae_early_stopping(train_loader, val_loader, self.model, patience=self.patience,
                                    num_epochs=self.num_epochs, lr=self.lr, writer=writer, verbose=self.verbose)
        print("Fitting done")
        
        # Compute train mean/std error on whole training data
        self.model.eval()
        whole_data_loader = DataLoader(dataset=train_data_win, batch_size=self.batch_size,
    ...
    def predict(self, test_data : pd.DataFrame, norm=2, sigma=3, if_shap=True, custom=False):
        ...
        test_loader = DataLoader(dataset=test_data_win, batch_size=self.batch_size,
                                 drop_last=False, pin_memory=True,
                                 shuffle=False)
        reconstr_errors = predict_lstm_var_test_scores(self.model, test_loader)
